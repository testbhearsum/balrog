...
platform_map = {
    ("Linux_x86-gcc3",): {"bouncer": "linux", "ftp": "linux-i686", "buildbot": "linux"},
    ("Linux_x86_64-gcc3",): {"bouncer": "linux64", "ftp": "linux-x86_64", "buildbot": "linux64"},
    ("Darwin_x86-gcc3-u-ppc-i386", "Darwin_ppc-gcc3-u-ppc-i386"): {"bouncer": "osx", "ftp": "mac", "buildbot": "macosx"},
    ("Darwin_x86-gcc3-u-i386-x86_64", "Darwin_x86_64-gcc3-u-i386-x86_64"): {"bouncer": "osx", "ftp": "mac", "buildbot": "macosx64"},
    ("WINNT_x86-msvc",): {"bouncer": "win", "ftp": "win32", "buildbot": "win32"},
    ("WINNT_x86_64-msvc",): {"bouncer": "win64", "ftp": "win64", "buildbot": "win64"},
}

deprecated_platform_map = {
    ("Darwin_x86_64-gcc3",): {"bouncer": "osx64", "ftp": "mac64", "buildbot": "macosx64"},
    ("Darwin_Universal-gcc3",): {"bouncer": "osx", "ftp": "mac", "buildbot": "macosx"},
}


def isMac(platform, version):
    if platform in ("osx", "mac", "macosx") or "Universal" in platform or "u-ppc-i386" in platform or "u-i386-x86_64" in platform:
        return True
...
def getPlatforms(platform, version):
    ...
    if isMac(platform, version):
        ...
        if (
            (version.startswith("3.5") and versionLt(version, "3.5.16"))
            or (version.startswith("3.6") and versionLt(version, "3.6.13"))
            or (version.startswith("4.0") and versionLt(version, "4.0b5"))
        ):
        ...
        if (
            (version.startswith("3.5") and versionGte(version, "3.5.16"))
            or (version.startswith("3.6") and versionGte(version, "3.6.13"))
            or version in ("4.0b5", "4.0b6")
        ):
...
if __name__ == "__main__":
    ...
    relData = {"name": options.name, "schema_version": SCHEMA_VERSION, "platforms": {}, "hashFunction": options.hash_func}
    ...
    if options.db:
        ...
        db.releases.insert(
            changed_by="generate-json.py", name=options.name, product=options.product, version=options.version, data=json.dumps(relData, separators=(",", ":"))
        )
